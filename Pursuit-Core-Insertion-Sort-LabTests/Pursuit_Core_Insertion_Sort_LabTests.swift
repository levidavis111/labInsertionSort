//
//  Pursuit_Core_Insertion_Sort_LabTests.swift
//  Pursuit-Core-Insertion-Sort-LabTests
//
//  Created by Benjamin Stone on 10/14/19.
//  Copyright © 2019 Benjamin Stone. All rights reserved.
//

import XCTest
@testable import Pursuit_Core_Insertion_Sort_Lab

class Pursuit_Core_Insertion_Sort_LabTests: XCTestCase {
    ...
    func testInsertionSorted() {
        for testCase in qOneAndThreeTestCases {
            let arrayUnderTest = insertionSorted(arr: testCase.collection, by: <)
    ...
    func testBubbleSortWithoutMovingNegatives() {
        for testCase in qTwoTestCases {
            let arrayUnderTest = insertionSortWithoutMovingNegatives(arr: testCase.collection, by: <)
            XCTAssertEqual(arrayUnderTest, testCase.sortedCollection, "Was expecting \(testCase.sortedCollection), but got \(arrayUnderTest)")
        }
    }
    
    func testInPlaceInsertionSort() {
        for testCase in qOneAndThreeTestCases {
            var testCaseCollection = testCase.collection
            let arrayUnderTest = insertionSort(arr: &testCaseCollection, by: <)
            XCTAssertEqual(arrayUnderTest,testCase.sortedCollection, "Was expecting \(testCase.sortedCollection), but got \(arrayUnderTest)")
        }
    }
    
    func testInsertionSortedLinkedList() {
        for testCase in qFourTestCases {
            let listUnderTest = insertionSorted(list: testCase.collection, by: <)
